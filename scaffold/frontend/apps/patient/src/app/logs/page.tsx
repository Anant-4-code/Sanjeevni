...
const EVENT_BADGES: Record<string, { label: string; bg: string; text: string; border: string; icon: typeof CheckCircle2 }> = {
  DOCTOR_VERIFIED: {
...
export default function ActivityLogsPage() {
  ...
  const filteredLogs = logs.filter((log) => {
    ...
    if (filter === "doses") return log.event_type === "DOSE_TOGGLED";
    ...
    <div className="max-w-5xl mx-auto w-full space-y-6">
      ...
      <div className="space-y-4">
        {filteredLogs.map((log) => {
          ...
          return (
            <div key={log.id} className="glass-card p-6 flex flex-col sm:flex-row sm:items-start justify-between gap-4">
              <div className="flex items-start gap-4">
                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0 border ${badge.border} ${badge.bg}`}>
                  <IconComp className={`w-5 h-5 ${badge.text}`} />
                </div>
                <div className="space-y-1.5 min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className={`text-[10px] font-mono font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full border ${badge.border} ${badge.bg} ${badge.text}`}>
                      {badge.label}
                    </span>
                  ...
                  <h3 className="font-bold text-base sm:text-lg text-[var(--fg)]">{log.title}</h3>
                  <p className="text-xs sm:text-sm text-[var(--fg-muted)] leading-relaxed">{log.details}</p>
                </div>
              </div>
        ...
        {filteredLogs.length === 0 && !loading && (
          <div className="glass-card p-12 text-center">
            ...
            <p className="text-xs text-[var(--fg-muted)]">No logs match the selected filter category.</p>
          </div>
        )}
      </div>
    </div>
