...
export default function App() {
  ...
  const [mode, setMode] = useState<"doctor" | "reception">("doctor");
  ...
  // Reception Registration Form State
  ...
  const [receptionSuccess, setReceptionSuccess] = useState<any>(null);
  ...
  // Reception Registration Submit
  const handleReceptionSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setReceptionSubmitting(true);
    try {
      const res = await fetch(`${API_BASE}/reception/register`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(receptionForm),
      });
      const data = await res.json();
      setReceptionSuccess(data);
      fetchQueue();
    } catch (e) {
      console.error("Reception registration failed:", e);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] text-[var(--fg)] flex flex-col font-sans transition-colors duration-300">
      {/* ── TOP HEADER (Corviin-Style Sticky Floating Glass Pill) ── */}
      <header className="sticky top-0 z-40 px-3 pt-2.5 pb-1">
        <div className="max-w-7xl mx-auto glass-panel rounded-2xl px-5 h-15 flex items-center justify-between shadow-sm">
          ...
          {/* Mode Switcher */}
          <div className="flex items-center gap-1 bg-[var(--bg-muted)] p-1 rounded-full border border-[var(--border)]">
            <button
              onClick={() => setMode("doctor")}
              className={`px-3.5 py-1 text-xs font-bold rounded-full transition-all ${
                mode === "doctor" ? "bg-[var(--fg)] text-[var(--bg)] shadow-xs" : "text-[var(--fg-muted)] hover:text-[var(--fg)]"
              }`}
            >
              🩺 Doctor Workspace
            </button>
            <button
              ...
              className={`px-3.5 py-1 text-xs font-bold rounded-full transition-all ${
                mode === "reception" ? "bg-[var(--fg)] text-[var(--bg)] shadow-xs" : "text-[var(--fg-muted)] hover:text-[var(--fg)]"
              }`}
            >
              🏥 Reception Intake
            </button>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            >
              {isRecording ? <><MicOff className="w-3.5 h-3.5" /> Recording...</> : <><Mic className="w-3.5 h-3.5 text-red-500" /> Dictate</>}
            </button>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-xs font-bold text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 bg-emerald-500/10 px-3 py-1.5 rounded-full hover:bg-emerald-500/20 transition-colors hidden md:inline-flex items-center gap-1"
            >
              <span>Patient Portal (Port 3000)</span>
      ...
      <main className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8 flex-1">
        {mode === "reception" ? (
          /* ── RECEPTION INTAKE MODE ── */
          <div className="max-w-3xl mx-auto glass-card p-8 space-y-6 border-2 border-[var(--border)]">
            <div className="space-y-1">
              <span className="text-xs font-mono uppercase tracking-[0.2em] text-[var(--fg-muted)]">
                01 // RECEPTION DESK
              </span>
              <h1 className="font-display text-3xl font-bold">New Patient Registration & Triage</h1>
              <p className="text-xs text-[var(--fg-muted)]">
                AI extracts chief complaints, estimates clinical acuity level, and queues the patient.
              </p>
            </div>
            ...
            {receptionSuccess && (
              <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 space-y-2">
                ...
                <p className="text-xs font-mono">
                  Token #{receptionSuccess.token_number} · Estimated Wait: {receptionSuccess.estimated_wait_minutes} mins
                </p>
              </div>
            )}
            ...
            <form onSubmit={handleReceptionSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-mono text-[var(--fg-muted)] block mb-1">PATIENT FULL NAME</label>
                  <input
                    type="text"
                    required
                    placeholder="e.g. Ramesh Kumar"
                    value={receptionForm.full_name}
                    onChange={(e) => setReceptionForm({ ...receptionForm, full_name: e.target.value })}
                    className="w-full px-3 py-2 text-xs border border-[var(--border)] rounded-xl bg-[var(--bg-elevated)] text-[var(--fg)] focus:outline-none"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-mono text-[var(--fg-muted)] block mb-1">AGE & GENDER</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-mono text-[var(--fg-muted)] block mb-1">PHONE NUMBER</label>
                  <input
                    type="tel"
                    required
                ...
                <div>
                  <label className="text-[10px] font-mono text-[var(--fg-muted)] block mb-1">EMERGENCY CONTACT</label>
                  <input
                    type="text"
                    placeholder="Name & Contact (Caregiver)"
                    value={receptionForm.emergency_contact_name}
                    onChange={(e) => setReceptionForm({ ...receptionForm, emergency_contact_name: e.target.value })}
                    className="w-full px-3 py-2 text-xs border border-[var(--border)] rounded-xl bg-[var(--bg-elevated)] text-[var(--fg)] focus:outline-none"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="text-[10px] font-mono text-[var(--fg-muted)] block mb-1">CHIEF COMPLAINT / SYMPTOMS</label>
                <textarea
                  rows={3}
                  required
                  placeholder="e.g. Severe chest pain radiating to left arm for 2 hours, shortness of breath"
                  value={receptionForm.chief_complaint}
                  onChange={(e) => setReceptionForm({ ...receptionForm, chief_complaint: e.target.value })}
                  className="w-full p-3 text-xs border border-[var(--border)] rounded-xl bg-[var(--bg-elevated)] text-[var(--fg)] focus:outline-none"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-3 bg-[var(--fg)] text-[var(--bg)] font-bold text-xs rounded-full hover:opacity-90 transition-opacity"
              >
                {receptionSubmitting ? "Registering & Triage Analyzing..." : "Submit Registration & Generate Token &rarr;"}
              </button>
            </form>
          </div>
        ) : (
          /* ── DOCTOR WORKSPACE MODE ── */
          <div className="space-y-8">
            {/* ── HERO BANNER & PROTOCOL STATUS ── */}
            <div className="glass-card p-6 sm:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
              <div className="space-y-2 z-10">
                ...
                <p className="text-sm text-[var(--fg-muted)] max-w-xl leading-relaxed">
                  Active Patient: <strong className="text-[var(--fg)] font-bold">{patientData?.patient?.full_name || "Vikram Singh"}</strong> ({patientData?.patient?.age || 52}y · {patientData?.patient?.gender || "Male"}) — Token #{patientData?.patient?.token_number || 14}. Real-time cross-specialist safety monitoring active.
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-6">
                      ...
                      <div className="glass-card p-5 space-y-3 border-2 border-[var(--border)]">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="text-xs text-[var(--fg-muted)] mt-0.5">
                              Prescribed by Dr. V. K. Rai (Cardiology) · 02:00 PM · 10 days remaining
                            </p>
                          </div>
                      ...
                      <div className="glass-card p-5 space-y-3 border-2 border-[var(--border)]">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="text-xs text-[var(--fg-muted)] mt-0.5">
                              Prescribed by Dr. S. K. Patel · 08:00 PM (Night)
                            </p>
                          </div>
                ...
                {/* ── TAB 2: MEDICAL RECORDS & DOCUMENT VAULT ── */}
